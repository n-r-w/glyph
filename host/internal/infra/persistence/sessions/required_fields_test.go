...
func TestLoadRejectsMissingRequiredNestedCoreFields(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:   "usage cached input tokens",
			record: requiredModelRecord,
			path:   []any{"response", "usage", "cachedInputTokens"},
		},
		{
			name:   "usage cache write tokens",
			record: requiredModelRecord,
			path:   []any{"response", "usage", "cacheWriteTokens"},
		},
		{
			name:   "usage reasoning tokens",
			record: requiredModelRecord,
			path:   []any{"response", "usage", "reasoningTokens"},
		},
		...
		{
			name:   "provider context provider ID",
			record: requiredModelRecord,
			path:   []any{"response", "content", 1, "providerContext", "providerId"},
		},
		{
			name:   "provider context API",
			record: requiredModelRecord,
			path:   []any{"response", "content", 1, "providerContext", "api"},
		},
		{
			name:   "provider context model",
			record: requiredModelRecord,
			path:   []any{"response", "content", 1, "providerContext", "model"},
		},
		{
			name:   "provider context payload",
			record: requiredModelRecord,
			path:   []any{"response", "content", 1, "providerContext", "payload"},
		},
		{name: "tool call ID", record: requiredModelRecord, path: []any{"response", "content", 2, "toolCall", "id"}},
		{
			name:   "tool call name",
			record: requiredModelRecord,
			path:   []any{"response", "content", 2, "toolCall", "name"},
		},
		{
			name:   "tool call arguments",
			record: requiredModelRecord,
			path:   []any{"response", "content", 2, "toolCall", "arguments"},
		},
		...
		{name: "tool result text kind", record: requiredToolResultRecord, path: []any{"result", "contents", 0, "kind"}},
		{
			...
			record: requiredToolResultRecord,
			path:   []any{"result", "contents", 0, "text"},
		},
		{
			name:   "tool result image media type",
			record: requiredToolResultRecord,
			path:   []any{"result", "contents", 1, "mediaType"},
		},
		{
			name:   "tool result image data",
			record: requiredToolResultRecord,
			path:   []any{"result", "contents", 1, "data"},
		},
	}
