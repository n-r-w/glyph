...
func TestReplayVersion2Tree(t *testing.T) {
	...
	content := fmt.Sprintf(
		`{"type":"session","version":2,"id":"stored","createdAt":"%s","cwd":%q}`+"\n"+
			`{"type":"entry","entry":{"type":"user","id":"root","parentId":null,`+
			`"createdAt":"%s","message":{"content":[{"kind":1,"text":"root"}]}}}`+"\n"+
			`{"type":"entry","entry":{"type":"model","id":"old","parentId":"root",`+
			`"createdAt":"%s","response":{"content":[],"outcome":1,"diagnostics":[]}}}`+"\n"+
			`{"type":"navigation","navigation":{"destinationId":"root"}}`+"\n"+
			`{"type":"entry","entry":{"type":"model","id":"new","parentId":"root",`+
			`"createdAt":"%s","response":{"content":[],"outcome":1,"diagnostics":[]}}}`+"\n"+
		...
		createdAt.Add(2*time.Second).Format(time.RFC3339Nano),
		createdAt.Add(3*time.Second).Format(time.RFC3339Nano),
		createdAt.Add(4*time.Second).Format(time.RFC3339Nano),
	)
	path := sessionPath(
		root,
		project,
...
func TestReplayRejectsInvalidVersion2Tree(t *testing.T) {
	...
	cases := map[string]string{
		"version one": `{"type":"session","version":1,"id":"stored",` +
			`"createdAt":"2026-08-29T01:00:00Z","cwd":%q}` + "\n",
		"forward parent": `{"type":"session","version":2,"id":"stored",` +
			`"createdAt":"2026-08-29T01:00:00Z","cwd":%q}` + "\n" +
			`{"type":"entry","entry":{"type":"user","id":"child",` +
			`"parentId":"later","createdAt":"2026-08-29T01:00:00Z",` +
			`"message":{"content":[]}}}` + "\n",
		"multiple payloads": `{"type":"session","version":2,"id":"stored",` +
			`"createdAt":"2026-08-29T01:00:00Z","cwd":%q}` + "\n" +
			`{"type":"entry","entry":{"type":"user","id":"root",` +
			`"parentId":null,"createdAt":"2026-08-29T01:00:00Z",` +
			`"message":{"content":[]}},"label":{"targetId":"root",` +
			`"label":"bad"}}` + "\n",
		...
		"invalid branch summary": validRootRecord() + `{"type":"navigation","navigation":{"destinationId":"root",` +
			`"branchSummary":{"type":"branch_summary","id":"summary",` +
			`"parentId":"root","createdAt":"2026-08-29T01:00:01Z",` +
			`"summary":"","firstEntryId":"root","lastEntryId":"root",` +
			`"provider":"provider","model":"model","reasoningChoice":"low"}}}` + "\n",
	}
