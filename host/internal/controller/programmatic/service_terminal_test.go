...
func TestFailureCodeForCommandEnforcesClosedSets(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:     "model selection accepts unavailable model category",
			command:  CommandSelectModel,
			proposed: FailureCodeModelUnavailable,
			expected: FailureCodeModelUnavailable,
		},
		{
			name:     "model selection accepts credential category",
			command:  CommandSelectReasoningChoice,
			proposed: FailureCodeCredentialUnavailable,
			expected: FailureCodeCredentialUnavailable,
		},
		{
			name:     "model selection accepts extension rejection category",
			command:  CommandSelectModel,
			proposed: FailureCodeExtensionRejected,
			expected: FailureCodeExtensionRejected,
		},
		{
			name:     "model selection accepts extension unavailable category",
			command:  CommandSelectReasoningChoice,
			proposed: FailureCodeExtensionUnavailable,
			expected: FailureCodeExtensionUnavailable,
		},
		{
			name:     "model selection accepts internal category",
			command:  CommandSelectModel,
			proposed: FailureCodeInternal,
			expected: FailureCodeInternal,
		},
		{
			name:     "model selection rejects agent-run model category",
			command:  CommandSelectModel,
			proposed: FailureCodeModelFailed,
			expected: FailureCodeInternal,
		},
