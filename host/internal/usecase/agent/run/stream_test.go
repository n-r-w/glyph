...
func TestApplyStreamEventBuildsOrderedTextState(t *testing.T) {
	...
	for _, event := range []StreamEvent{
		{Kind: StreamEventContentStart, Position: 1, Content: model.Content{Kind: model.ContentText}},
	...
	assert.Equal(t, "Hello", partial.Content[1].Text)
}
...
func TestApplyStreamEventBuildsOrderedReasoningState(t *testing.T) {
	...
	partial := model.Response{}
	for _, event := range []StreamEvent{
		{Kind: StreamEventContentStart, Position: 0, Content: model.Content{Kind: model.ContentReasoning}},
		{Kind: StreamEventTextDelta, Position: 0, Content: model.Content{Kind: model.ContentReasoning}, Delta: "why"},
		{Kind: StreamEventContentEnd, Position: 0, Content: model.Content{Kind: model.ContentReasoning}},
		{Kind: StreamEventContentStart, Position: 1, Content: model.Content{Kind: model.ContentText}},
		{Kind: StreamEventTextDelta, Position: 1, Content: model.Content{Kind: model.ContentText}, Delta: "answer"},
		{Kind: StreamEventContentEnd, Position: 1, Content: model.Content{Kind: model.ContentText}},
	} {
		require.NoError(t, applyStreamEvent(&partial, event))
	}
	...
	assert.Equal(t, "why", partial.Content[0].Text)
	assert.True(t, partial.Content[0].Final)
	assert.Equal(t, model.ContentText, partial.Content[1].Kind)
	assert.Equal(t, "answer", partial.Content[1].Text)
}
...
func TestServiceStateIsolatesNestedToolPreviewValues(t *testing.T) {
	...
	service := New("test", model.Descriptor{}, nil, hookrunner.New(nil, nil, nil), nil, nil)
...
func TestServiceTerminalStreamEventClearsToolCallPreview(t *testing.T) {
	...
	require.NoError(t, service.applyStreamEvent(StreamEvent{
		Kind: StreamEventDone, Response: model.Response{Outcome: model.OutcomeLength},
	}))
...
func TestApplyStreamEventRejectsEventsAfterTerminal(t *testing.T) {
	...
	require.NoError(t, applyStreamEvent(&partial, StreamEvent{
		...
		Response: model.Response{Outcome: model.OutcomeStop},
	}))

	err := applyStreamEvent(&partial, StreamEvent{
		Kind:     StreamEventError,
		Response: model.Response{Outcome: model.OutcomeFailed, ErrorMessage: "failed"},
	})

	require.ErrorContains(t, err, "already terminated")
	assert.Equal(t, model.OutcomeStop, partial.Outcome)
}
