...
The audit and corrections satisfy the [ticket](specs/features/initial/phases/07.1-architecture-audit-and-correction/ticket.md). All confirmed architecture, behavior, cleanup, portability, documentation, and test-quality findings are closed. The [technical solution](specs/features/initial/phases/07.1-architecture-audit-and-correction/solution.md) contains the implementation and verification evidence. PHS-07 may resume.

Documents:
- [Ticket](specs/features/initial/phases/07.1-architecture-audit-and-correction/ticket.md)
- [Correction status and evidence](specs/features/initial/phases/07.1-architecture-audit-and-correction/solution.md#implementation-evidence)
- [Remaining-corrections decisions and evidence](specs/features/initial/phases/07.1-architecture-audit-and-correction/solution.md#remaining-corrections)
- [TUI ownership gap](specs/features/initial/phases/07.1-architecture-audit-and-correction/tui-ownership-gap.md)
...
Move the complete compaction policy and composition coordination into the policy-owning compaction extension. The policy includes triggering, context sizing and usage observations, retained-history budget and boundary selection, summary preparation and generation, and compaction-specific overflow handling. Preserve ordered request handlers, immutable original and composed current values, independent request/result updates, generation bypass for a complete result, ordered result handlers, explicit cancellation, and custom complete replacement. Keep durable replacement entries, original history, restoration and navigation projection, structural session checks, publication, and accounting with the session and Host runtime owners. Agent Core consumes prepared history and has no compaction policy or compaction-specific orchestration.

Units 0 through 3 and the session entry/projection work remain implemented. Unit 4 is an unaccepted Host-owned compaction checkpoint retained for comparison and migration evidence; it is not the target design. The minimal replacement integration contract remains [technical design work](specs/features/initial/phases/06-context-compaction-retry-control/ticket.md#qst-01-minimal-compaction-integration-contract). The ownership migration and bundled compaction extension remain pending, so PHS-06 is not complete. The [implementation evidence](specs/features/initial/phases/06-context-compaction-retry-control/solution.md#implementation-evidence) records the checkpoint and its open findings. Follow the [migration plan](specs/features/initial/phases/06-context-compaction-retry-control/migration-plan.md), starting with PHS-06.1 integration design, to continue from checkpoint `690827f`.
