...
- PRB-01: The standard TUI editor currently accepts a single flattened line and a small fixed key set. It has no multiline editing, history, completion, attachments, clipboard semantics, external editor, direct shell action, reusable selector behavior, hotkey discovery, suspension, or complete terminal restoration contract.
...
- Required behavior: the standard TUI preserves text and attachments, exposes completion and history, queues the selected message kind, and restores terminal modes after suspension or exit.
...
In scope:
- ISP-01: Editor, completion, queued-input, selector, keybinding, hotkey, lifecycle, and error requirements FRQ-21 through FRQ-43 in [`standard-tui.md`](../standard-tui.md).
...
- DEP-01: [PHS-12.2](12.2-standard-tui-viewport-navigation.md) must meet all acceptance criteria.
...
- FRQ-09: Send a Host direct-shell command that requests model-visible-output or model-hidden-output, and render the Host result or error. The standard TUI shall not execute shell behavior.
- FRQ-10: Keep clear, abort, and exit as distinct actions and preserve unsent DEF-01 when aborting a run.
- FRQ-11: Make every DEF-03 support item movement, page movement, confirm, cancel, selected-item visibility, and filtering for searchable sources.
- FRQ-12: Restore editor text, viewport position, and focus after selector or dialog cancellation.
- FRQ-13: Apply configurable keys from [`tui-defaults.md`](../tui-defaults.md) and expose active command and key help.
- FRQ-14: Recompute editor wrapping, selector geometry, and focused-region placement after terminal resize.
- FRQ-15: Restore application input modes, viewport, editor, and focus after suspension and resume.
- FRQ-16: Disable alternate screen, mouse reporting, bracketed paste, focus reporting, and application keyboard modes after normal exit, UI process failure, and Host termination.
...
- NFQ-02: Clipboard, external-editor, completion, shell, and attachment errors must remain visible and leave the editor usable.
...
- DLV-01: Multiline editor, history, completion, attachments, clipboard, external-editor, Host direct-shell command dispatch and result presentation, and queued-input presentation.
- DLV-02: Reusable selector and dialog interaction behavior.
- DLV-03: Configurable action dispatch, hotkey help, suspension, resume, resize, and terminal restoration.
...
- ACC-05: The standard TUI dispatches a Host direct-shell command and renders both model-visible-output and model-hidden-output outcomes without executing a shell process.
- ACC-06: Every built-in selector passes movement, paging, filtering, confirmation, cancellation, and focus-restoration tests.
- ACC-07: User key configuration replaces defaults and hotkey help shows the effective bindings.
- ACC-08: Suspension, normal exit, UI failure, and Host termination leave all application-owned terminal modes disabled.
- ACC-09: Terminal sizes down to one row and one column do not panic.

## Overengineering and Overspecification Considerations

This ticket extracts user-visible terminal behavior and leaves Agent Core queue semantics, Host command discovery, session operations, and provider selection in their owning use cases. It does not copy Pi editor classes, settings files, or action dispatch architecture.
...
No editor widget, clipboard library, fuzzy matcher, external-editor command resolution, shell syntax, or terminal keyboard protocol is selected by this ticket.
...
- REF-05: [Pi usage](https://github.com/badlogic/pi-mono/blob/main/packages/coding-agent/docs/usage.md) - behavioral source for completion, attachments, external editor, shell actions, and queued messages.
