...
Glyph provides an independently owned Go platform whose host, agent core, interfaces, and extension contracts can evolve in one codebase.

## Goal

Deliver an independent Go agent platform with a TUI-free agent core, an extension-managing host, a standard terminal coding interface, and transport-neutral programmatic control.
...
- An extension requests interaction or sends a notification through the Glyph host to an attached interface.
...
- Glyph host, agent core, standard TUI, and programmatic control.
...
- A universal extension renderer shared by the standard TUI and future interfaces.
- Windows support.
...
- The standard TUI shall depend on the Glyph host and agent core public contracts; neither the host nor the agent core shall require the standard TUI.
...
- An extension shall be able to request interaction through an attached interface without prescribing how that interface presents the request.
- When no interface is attached, an interaction request shall fail explicitly; the host shall not fabricate a successful result.
- An extension shall be able to send a notification through the host to an attached interface.
- The attached interface shall choose how to present a notification. A programmatic controller shall receive it as an event.
- Notification delivery shall succeed when the host transfers the notification to the attached interface and shall report an error if transfer fails; success shall not require presentation, human observation, or a user response.
- Notification delivery without an attached interface shall fail explicitly.
...
- Extensions shall be able to register commands that attached interfaces can discover and invoke; user-facing command syntax shall belong to the interface.
...
- The bundled resource extension shall convert skills and context files into system instructions and model context and make prompt templates available through attached interfaces.
...
- The Glyph host shall provide provider implementations with generic credential storage and interaction through an attached interface.
...
- Reload shall cover host settings, provider registrations, extension runtimes, and resource contributions.
...
- The standard TUI shall provide an interactive terminal interface for the standard coding agent without owning agent-core behavior.
...
- When the Glyph host performs environment reload, the attached standard TUI shall reload its themes and key bindings.
...
- The standard TUI shall expose terminal-specific extension capabilities only while it is attached to the Glyph host.
...
- A TUI-specific renderer shall not be required to work in another interface. Future interfaces may expose their own extension-rendering contracts.
...
No technical solution is selected in this document. The mechanism connecting the Glyph host, agent core, attached interfaces, and extension runtimes is deferred to technical design. Runtime extension feasibility is analyzed in `docs/artefacts/go-extension-feasibility.md`.
...
| `extensions/mcp-wrapper/index.ts` | Agent and Tool Runtime: tool registration; Extensions and Interfaces: commands |
...
| `extensions/codex-fast/index.ts` | Core Extension Capabilities: provider-request middleware; Extensions and Interfaces: commands |
...
| `extensions/completion-sound/index.ts` | Core Extension Capabilities: lifecycle events; Extensions and Interfaces: notifications |
| `extensions/cmux/index.ts` | Core Extension Capabilities: lifecycle and tool-result events; Extensions and Interfaces: notifications |
| `extensions/main-agent-selection/index.ts` | Extensions and Interfaces: commands; TUI Extension Capabilities: custom content |
...
| `extensions/structured-prompt/index.ts` | Extensions and Interfaces: commands and interaction requests; TUI Extension Capabilities: custom content and editor integration |
| `extensions/ask-llm/index.ts` | Extensions and Interfaces: commands; Core Extension Capabilities: configured-model requests; TUI Extension Capabilities: custom content |
