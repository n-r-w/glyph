...
- During a user-invoked extension command, the extension shall be able to initiate and cancel session creation, resumption, forking, cloning, and tree navigation through Host. Automatic initiation of these operations from ordinary lifecycle handlers is outside the initial scope.
...
- The compaction extension shall own the complete compaction policy and its request/result composition: trigger decisions, context sizing, retained-history budget and boundary selection, summary input preparation, summary generation, and compaction-specific response to provider context overflow.
- Before manual, threshold, or overflow-recovery compaction, the policy-owning extension shall establish one immutable original request and an equal current request.
- Compaction request handlers shall run sequentially. Each handler shall receive the immutable original request, the current request, and the current result when one exists. One state update shall independently preserve or replace the current request and preserve, set, replace, or clear the current result. Cancellation shall be terminal.
- Each later request handler shall receive the unchanged original request and the current request and result returned by the preceding handler. A handler shall be able to derive its update from the original request, the current state, or both.
- A complete result supplied through request-handler composition shall bypass generator lookup and invocation. When request handlers finish successfully without a result, the policy-owning extension shall invoke the single registered generator. The bundled extension shall supply the default generator, and a custom extension shall be able to replace it. Zero or multiple generators shall fail without a Host fallback or load-order selection.
- Result handlers shall run sequentially after a result exists. Each shall receive the immutable original request, final current request, immutable original result, and current result from preceding result handlers and shall preserve or replace the result or cancel compaction. Cancellation shall be terminal.
- The Glyph host shall provide extension transport and invocation, cancellation, session-bound access, model access, durable history mutation, committed-entry publication, and existing accounting. Host shall not implement duplicate compaction composition, trigger, sizing, retained-boundary, generator-selection, or compaction-specific overflow policy.
- The session owner shall validate session identity, active-branch membership, replacement-entry structure, source accounting, and tool-call/result integrity before it atomically appends a compaction entry. These checks shall not select or revise compaction policy.
- The target integration contract shall keep Agent Core unaware of compaction and shall use the existing extension runtime and session model. The exact mechanism through which the policy-owning extension invokes participating handler and generator capabilities remains PHS-06 technical design work.
- A handler error, invalid action, cancellation, or unavailable compaction capability shall stop compaction under the [extension failure rule](#extension-failure-rule) without appending a compaction entry, invoking a later handler, retrying the extension, or invoking another compaction implementation as fallback.
...
- The policy-owning compaction extension shall decide when remaining model context requires automatic compaction and how to recover from compaction-specific provider context overflow.
- Manual compaction shall accept user instructions and shall use the same extension-owned request, generator, and result composition as automatic compaction.
- Agent Core shall consume prepared provider-neutral history and shall have no compaction policy, compaction-specific orchestration, plugin transport, persistence, or session-projection responsibility.
- Glyph shall distribute a compaction extension enabled by default for the standard coding agent. The extension shall own the complete standard compaction policy, including use of preceding summaries and configured-model requests. A user extension shall be able to replace it through the ordinary extension lifecycle. Host shall provide runtime and session services without a compaction-policy fallback.
- Context compaction shall append a replacement entry that replaces an older context prefix in model-visible context while preserving the remaining suffix unchanged and retaining the original session entries.
- The bundled compaction extension shall use a configurable recent-history token budget with a default of 20,000 tokens. The selected boundary shall retain complete messages and shall not separate a tool result from its tool call. Response-budget accounting shall remain separate.
- Repeated compaction shall produce an updated summary from the preceding summary and the newly summarized part of the previously retained suffix. It shall not summarize the complete original history again. The next prepared history shall contain the updated summary and the retained suffix, not a chain of preceding compaction summaries. Session restoration shall reproduce this active-branch context.
...
| Session-tree navigation and branch summarization | Context and Sessions; Session Interaction; Extension Capabilities | PHS-05, PHS-05.2 | An extension replaces branch summarization without an unused model dependency, and clients receive handler error causes without losing branches. |
| Extensible compaction and retry | Context and Sessions; Retry; Extension Capabilities | PHS-06 | Extensions compose compaction requests and results under extension-owned policy, Host owns retry coordination and session services, and a client observes both operations. |
| Extension lifecycle, persisted state, and active selection | Extensions and Glyph Clients; Extension Capabilities | PHS-07 | An extension recovers persisted active-branch state through its public context and changes the active model through ordered selection handlers. |
...
| Commands, session control, interactions, notifications, and events | Extensions and Glyph Clients; Extension Capabilities | PHS-10 | A client invokes an extension command that controls a session and exchanges events, interactions, or notifications. |
