...
- FRQ-02.3.1: Loss of a provider extension shall preserve the extension runtime error and shall produce the same provider-neutral failure information for bundled and separately delivered providers. It shall stop the model execution without retry under the [extension failure rule](../../prd.md#extension-failure-rule), rather than being treated as a transient response from the model provider.
