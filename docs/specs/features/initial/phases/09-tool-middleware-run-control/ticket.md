...
- FRQ-01: Add pre-execution tool handlers for allow, reject, and input modification. Each modifying handler shall receive the immutable original tool call and the current tool call returned by preceding handlers.
- FRQ-01.1: An explicit tool-policy rejection shall become a model-visible error result with a closed Glyph category and complete error text. It shall not fail the parent agent run automatically. A pre-execution handler error or invalid action shall instead follow the [extension failure rule](../../prd.md#extension-failure-rule): stop the run, block that tool, invoke no later handler, expose the complete error text, and perform no retry.
...
- FRQ-02.1: Tool-result transformations shall preserve the error category and complete error text unless a handler explicitly replaces the complete current tool result. A result-handler failure shall stop the run under the extension failure rule; it shall not repeat or undo the completed tool execution.
...
- ACC-01.1: A pre-execution handler error prevents that tool from running and fails the agent run with complete error text. A tool-result handler error fails the run without repeating the completed tool. Both failures invoke no later handler and cause no retry.
