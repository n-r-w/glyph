...
- Disposition: The separate [process correction](solution.md#process-cancellation-evidence) implements O76-1 and O77-1 after the verified FND-18 through FND-21 commit. Main source review and repeated native/Linux verification are complete; independent integrated acceptance remains pending. The retained callback error reached the caller after the original delayed completion, so this is distinct from FND-21.
- Verification: The original process and descriptor observations established the surviving child after a successful signal. The permanent fork and final-flush regressions produced executable assertion RED, then GREEN. Local native macOS race and actual Linux checks cover inherited and redirected descendants, caller cancellation, timeouts, consumer failures and output completion after shell exit. [The evidence record](solution.md#process-cancellation-evidence) also preserves the Linux observer correction and the stale native timeout assertion failures. Whole-product acceptance is not claimed.
