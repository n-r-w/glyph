...
The first main coverage run failed in [TestStandardTUIHostSmoke](../../../../../../host/internal/app/standard_tui_app_test.go) while waiting for `Updated:`. The preceding `/new` received an operation-active rejection. That fixture waits for model text `Request complete.` rather than settlement before sending `/new`; model text does not establish admission release. With no source change, a repeated `task test-coverage` passed at 84.3%. The failed attempt remains recorded, and the smoke fixture's synchronization gap remains for verification correction. It does not alter the FND-18 through FND-21 RED/GREEN evidence.
...
After U6/U7 preservation re-entry, resume the separately approved U8 fixture correction. `prepareSubmit.Release` publishes Idle after `RunPrepared` releases admission, so a checkpoint during Running identifies a valid post-run barrier. With FND-23 corrected, a checkpointed confirmed replacement also establishes readiness for the next input. Do not use model text or arbitrary output as substitutes. Fresh independent whole-product checks/review and explicit final acceptance remain mandatory.
