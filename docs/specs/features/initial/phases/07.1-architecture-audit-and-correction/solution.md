...
##### U6 runtime completion evidence

D1 implementation and main-agent verification address [FND-15](audit.md#fnd-15-runtime-cleanup-discards-sdk-completion-errors) above `e4ef9d1acbd82746962644cf7eaee14dc21b83eb`. Independent integrated review and final acceptance remain pending.

[SDK connection completion](../../../../../../sdk/plugins/extension/v1/host.go) exposes `CompletionFailures` after producer and transport joins. It contains undelivered Owner/Writer sources and independent `CloseSend` failures. Writer and receive results describe connection stopping and remain in `Connection.Close`, not the runtime cleanup projection. Cleanup filtering does not inspect declared sources. Mixed cleanup trees retain their original values. Delivery classification is selected before source aggregation.

The [runtime adapter](../../../../../../host/internal/infra/plugins/extension/runtime/runtime.go) implements `ExtensionRuntime.Close() error`. It also reports the original full `Connection.Close` diagnostic through `slog.ErrorContext`, without returning connection-stop observations as application-fatal. [Diagnostic selection](../../../../../../host/internal/infra/plugins/extension/runtime/diagnostic.go) suppresses only pure shutdown trees and preserves original mixed causes. Selection tests assert values and text, not logs. `TestIdleProtocolFailureRetainsNonfatalDiagnostic` checks an idle invalid envelope whose full close result remains observable while `CompletionFailures` stays nil. Early registration, execution, handler and lifecycle cleanup leaves diagnostics available for repeated final collection. The [Host runtime owner](../../../../../../host/internal/usecase/host/extensionruntime/service.go) records each state completion once, including rejected and replaced states and registration failures. Its final result reaches the established application cleanup paths. Returning diagnostics does not request Host shutdown.

- [Actual application regression](../../../../../../host/internal/app/runtime_completion_integration_test.go) failed before correction because application completion returned nil despite undelivered rejection text. Its three cases cover undelivered rejection, delivered rejection remaining nonfatal, and registration-time early close followed by final collection. All retain ordinary provider completion after successful mode-specific reporting. Twenty uncached race repetitions passed.
- [SDK completion regression](../../../../../../sdk/plugins/extension/v1/context_send_failure_integration_test.go) covers Failed and Completed Host work, mixed cancellation/source identity, independent `CloseSend` identity, transport classification and repeated diagnostics access. One hundred uncached race repetitions passed. This generated-stream test supplies the independent cleanup failure; the application test uses the actual runtime and process.
- [Owner completion regression](../../../../../../host/internal/usecase/host/extensionruntime/completion_test.go) covers registration failure, rejection, replacement and repeated final collection without duplication. Runtime continuation tests retain ordinary `HandlerError`, rejection, failure and cancellation behavior.

Main repeated `task fmt`, `task fix_dry_run` with no proposals, `task lint`, `task test`, `task itest`, `task test-coverage` and `task build`; all passed at 84.2% coverage against 80.0%. Broad uncached unit and integration races passed for `./host/... ./internal/operation ./plugins/... ./sdk/...`. Twenty actual-app race repetitions passed all 60 leaf cases. One hundred SDK repetitions passed all 300 Failed/Completed source cases. Diagnostic selection, idle protocol observation, transport-cleanup synchronization and runtime removal/reporting tests each passed 100 race repetitions. Two generation runs preserved all 897 file hashes. Supplemental unit-tag lint retains the same 165 baseline findings and is not required-lint success. The AST scan retains 154 adjacent root-production assertions. No dependency, protocol field, queue, retry, adapter source map or implementation assertion changed.

##### U6 producer preservation

This correction above `cb3a8f6b75bea1129e1bb915fe080eef8c792017` addresses [FND-18 through FND-21](audit.md#fnd-18-navigation-drops-accumulated-diagnostics-on-later-failure). O74-1 explicitly includes the additional bash producer loss in the same complete corrective unit. O75-1 authorizes a complete separate commit for this correction, followed by a separately designed process-cancellation correction for FND-22. D1/D2 and earlier retention remain intact. Main-agent source and verification checks below are complete. Independent integrated review and final acceptance remain pending. PHS-07.1 is blocked and PHS-07 stays paused.

| Producer | Preservation behavior and executable evidence |
| --- | --- |
| Sessiontree | Each internal issue retains its original cause. `navigate` joins accumulated issue sources on later failure. Both `NavigateUI` and `NavigateProgrammatic` return that error. [Twelve composed cases](../../../../../../host/internal/usecase/host/sessiontree/failure_retention_test.go) failed before correction and pass afterward, covering summary, validation, pre-commit and three cancellation points. Primary navigation categories, explicit handler cancel decisions and committed publication remain unchanged. |
| Extension runtime | `executionFailure`, `executionError` and `handlerOperationError` retain already received errors and independent progress causes alongside cancellation. [Selection tests](../../../../../../host/internal/infra/plugins/extension/runtime/cancellation_error_test.go) establish received-before-cancel ordering. [The real runtime callback case](../../../../../../host/internal/infra/plugins/extension/runtime/runtime_test.go) cancels before returning an independent progress failure. Both produced assertion RED/GREEN. Diagnostic-bearing gRPC Canceled retains its original status and text. |
| Extension SDK | `operationOutcome` produces Canceled only for pure cancellation trees. Ordinary mixed failures produce the established INTERNAL Failed outcome with their original source. This applies to registration, handler, execute, cancellation, Host work and Host cancellation prepared operations. [Classifier tests](../../../../../../sdk/plugins/extension/v1/server_test.go) and [SDK Owner collection](../../../../../../sdk/plugins/extension/v1/retention_integration_test.go) produced assertion RED/GREEN. Explicit FailureError and context-operation categories keep precedence; Completed HandlerError remains data with a declared source. |
| Bundled bash | O74-1 retains callback failures in the existing `outputSink`, independently of the context's first cancellation cause. `outputSink.result` joins them after process I/O and flush producers finish. `outputStore.finish` retains output-file failures. The usecase wraps causes, the controller preserves cancellation-bearing errors, and the corrected SDK classifier declares their sources. [The real process/usecase regression](../../../../../../plugins/extension/tools/internal/usecase/tools/bash/timeout_integration_test.go) produced assertion RED/GREEN. [Controller cases](../../../../../../plugins/extension/tools/internal/controller/extension/bash_test.go) preserve pure and mixed cancellation errors. |

The navigation, runtime selection/overlap, SDK Owner and printf-only bash schedules passed 100 uncached race repetitions. The final printf-only fixture also produced assertion RED against the exact pre-correction bash production file and uncached GREEN after exact restoration.

At completion of this producer-preservation unit, [FND-22](audit.md#fnd-22-bash-group-cancellation-can-leave-a-child-running) blocked whole-product acceptance. The descendant timeout was not a fixture-only exclusion. O75-1 authorized completing the error-preservation commit before the separate process-lifetime correction. This producer-preservation record includes no termination correction.

Main inspected all seven changed production files, their callers, both navigation client implementations, all six SDK outcome producers, and the bash output lock and join path. Main repeated `task fmt`, `task fix_dry_run` with no proposals, `task lint`, `task test`, `task itest`, `task test-coverage` and `task build`. Coverage completed at 84.3% against 80.0%. Both broad `-race -count=1` suites passed for `./host/... ./internal/operation ./plugins/... ./sdk/...`; integration used `-tags=integration -p 1 -parallel 1`. Two generation runs preserved all 899 hashes. Supplemental unit-tag lint retains exactly the previous 165 normalized findings and is not required-lint success. The 75-package root graph remains acyclic and contains 72 production packages. No public schema, dependency or implementation assertion changed.

Main also verified exact RUN/PASS records for 100 repetitions of each corrected schedule: 1,200 navigation leaves, 200 received-terminal leaves plus 100 transport-diagnostic cases, 100 real-runtime progress-overlap cases with rejection/failure and pure-cancellation controls, 200 SDK classifier cases, 300 SDK Owner leaves, 100 real-bash source cases and 200 controller pure/mixed leaves. An initial untagged bash-controller selector ran no tests and is excluded; the corrected integration-tagged command ran all 200 leaves.

The first main coverage run failed in [TestStandardTUIHostSmoke](../../../../../../host/internal/app/standard_tui_app_test.go) while waiting for `Updated:`. The preceding `/new` received an operation-active rejection. At that revision, the fixture waited for model text `Request complete.` rather than settlement before sending `/new`; model text does not establish admission release. With no source change, a repeated `task test-coverage` passed at 84.3%. The failed attempt remains recorded. The later [U8 fixture correction](u8-evidence.md#smoke-fixture-correction) adds the source-backed admission and replacement barriers. It does not alter the FND-18 through FND-21 RED/GREEN evidence.

#### Separate process-cancellation correction

O75-1 authorizes a separate correction for [FND-22](audit.md#fnd-22-bash-group-cancellation-can-leave-a-child-running) after producer-preservation commit `2df9e86c8b96c02a79c4113243c3a67564eff898`. O76-1 approves the process/output lifetime design, verification and sequential assignments below, including the subsequent U8 smoke-fixture correction. O77-1 keeps cancellation monitoring active through the final UTF-8 flush. These approvals do not reopen the error-preservation commit.

##### Process and output completion

The [process adapter](../../../../../../plugins/extension/tools/internal/infra/process/bash/service.go) owns this correction. The bash usecase retains execution timeout and cancellation-cause policy. The extension controller and SDK retain command and outcome categories.

The observed blocker is a coupled join. With `streamWriter` assigned directly to `Cmd.Stdout` and `Cmd.Stderr`, [Go's Cmd.Wait](https://pkg.go.dev/os/exec#Cmd.Wait) waits for both shell exit and internal output-copy workers. A surviving child can therefore prevent the caller from observing shell completion. Repeating signals only while output remains open would also make cancellation depend on whether descendants redirect their output.

The approved correction separates those lifetimes:

1. The process adapter owns stdout/stderr `os.Pipe` endpoints and two copy workers. The child receives file endpoints, so `Cmd.Wait` observes shell exit without waiting for those copy workers. `Cmd.Wait` remains the sole shell waiter. Do not combine it with `Process.Wait` or use `StdoutPipe`, whose documented `Wait` behavior closes the read pipe before this design's explicit join.
2. Caller cancellation, timeout, or a callback/output failure still requests immediate group SIGKILL. Once the shell has exited, cancellation cleanup sends a final SIGKILL to the original group before completing output cleanup. This final signal does not depend on whether a child still holds an output pipe.
3. Cancellation monitoring remains active while output workers or the final UTF-8 flush are pending, including when the shell exits first. Join both copy workers, flush stdout followed by stderr, then close `outputDone`, join cancellation work and collect the result. O77-1 corrects the earlier join-before-flush order: a flush callback can itself fail and request group cancellation. Close every owned pipe endpoint on its corresponding start, copy, or cleanup path.
4. An absent group is already stopped. Preserve other signal, copy, close, callback and capture errors alongside the original cancellation or timeout cause. Keep FND-21's sink-owned callback cause. Preserve pure cancellation status, timeout results, normal nonzero exit results, raw output capture, UTF-8 projection and delivery order.
5. The correction adds no polling loop, retry policy, process supervisor, wrapper executable, dependency, queue, public field or new retention owner. Do not wait for group disappearance with signal zero; a stopped zombie is not running work. Do not claim containment of descendants that deliberately leave the owned group or of arbitrary hostile fork loops.

The final signal follows shell exit, so that shell cannot create another child afterward. This addresses the observed survivor rather than only releasing its pipes. Ordinary shell descendants still need executable verification; the exact kernel fork/signal ordering and universal containment are not assumptions of the design.

##### Alternative considered

A ticker could repeat group SIGKILL while the old `Cmd.Wait` remains pending. It is a smaller edit but couples termination to output-copy completion. A surviving child with redirected output can outlive that wait. A pipe holder can prolong repeated signaling for reasons unrelated to runnable group members. This option is not recommended. Closing pipes or using `WaitDelay` alone is not an alternative termination solution because work can remain alive.

##### Verification and execution boundary

- Preserve the recorded assertion reproductions of FND-22, including `TestFlushCancellationKillsDescendant`, whose live-child assertion failed after the final UTF-8 callback requested cancellation. Permanent process regressions must check actual descendant termination and operation completion, not only pipe closure. Exercise inherited and redirected output, caller cancellation and timeout, shell exit before output completion, and normal completion. Use the original `printf started; sleep 30` schedule as repeated race coverage. A Go test timeout is not RED evidence; use an explicit completion assertion with cleanup of the exact test group.
- Reuse the bash integration suite and FND-21 tests. Verify full callback, signal and output causes and the existing output-order/UTF-8/spill behavior. Do not add production abstractions solely to control test scheduling.
- Run native macOS uncached race regressions and required project tasks. The existing Debian bookworm Docker image executes Bash 5.2.15 and `sleep` on Linux ARM64. It does not contain `ps`; Linux process observations must not assume that command is installed. Build the Linux integration binary from this checkout and execute it in that image with temporary files, without copying the checkout, pulling an image or installing dependencies. Cross-compilation alone is not signal-behavior evidence.
- Approved implementation assignment is one new sequential `SubAgentCoderComplex` instance in `unit_5`, which owns the bash process correction. Do not reuse the completed producer-preservation session. Main checks RED/GREEN, sources, joins and raw outputs, then creates one separate corrective commit. Unchanged U1 through U4 re-entry precedes it; U6/U7 re-entry follows it.
- During U8 verification correction, repair the recorded `TestStandardTUIHostSmoke` synchronization gap at its fixture. The fixture must wait for the specific completed operation or confirmed replacement, not model text or arbitrary output. Do not change TUI behavior, add a sleep, or weaken the `Updated:` assertion. This test-only correction uses the established U8 verification owner and a separate complete corrective commit.
- Fresh independent whole-product checks and architecture review follow U8. PHS-07 remains paused and explicit final user acceptance remains mandatory.

##### Process-cancellation evidence

The [process owner](../../../../../../plugins/extension/tools/internal/infra/process/bash/service.go) and its [owned pipes](../../../../../../plugins/extension/tools/internal/infra/process/bash/pipe.go) implement O76-1 and O77-1 above `2df9e86c8b96c02a79c4113243c3a67564eff898`. Main source review and repeated verification below are complete. The bash usecase, controller and SDK production source are unchanged. The TUI smoke fixture is unchanged.

The first native `TestCancellationForkRace` run failed explicit completion and live-child assertions before production changes. At attempt 8, the operation remained incomplete after one second and its owned group retained a live descendant. Test cleanup killed only that group and joined the operation. The same command passed after correction. The final UTF-8 flush regression then exposed the join-before-flush gap. Its live-child assertion failed after the callback error reached Run's caller. O77-1 changed the completion order, and the exact regression passed. Neither RED was a setup failure or Go test timeout.

| Regression | Observed contract |
| --- | --- |
| [Fork cancellation](../../../../../../plugins/extension/tools/internal/infra/process/bash/cancellation_integration_test.go) | Inherited and redirected output have separate 1-second assertions for Run completion and observation of no live group members before test cleanup signals. |
| [Output lifetimes](../../../../../../plugins/extension/tools/internal/infra/process/bash/lifetime_integration_test.go) | Caller cancellation, timeout causes and consumer failures cover inherited output, redirected output and shell exit before output completion. Normal nonzero shell exit still drains later descendant output. The original `printf started; sleep 30` command has separate repeated coverage. |
| [Final UTF-8 flush](../../../../../../plugins/extension/tools/internal/infra/process/bash/flush_cancellation_integration_test.go) | A consumer failure on the final replacement character retains its source and terminates an ordinary redirected descendant. |
| [Usecase timeout](../../../../../../plugins/extension/tools/internal/infra/process/bash/timeout_integration_test.go) | The real timeout owner terminates inherited and redirected descendants, completes within the assertion bound, and preserves exact timeout text, exit status, bounded result and progress order. |
| [Pipe failures and cleanup](../../../../../../plugins/extension/tools/internal/infra/process/bash/pipe_integration_test.go) | Read and close errors remain independent causes. Start-failure cleanup closes all four endpoints. |
| [Test group cleanup](../../../../../../plugins/extension/tools/internal/infra/process/bash/cleanup_integration_test.go) | Cleanup terminates a live member and accepts zombie-only `EPERM` only after observing no live members. Native cleanup observations remain bounded after the test context is canceled. |

Five of 100 earlier native timeout checks failed because their exact-error assertion excluded an independent group-signal `EPERM`. The relocated timeout regression accepts only the complete timeout diagnostic, or that diagnostic plus one typed group-signal `EPERM` with its full context on Darwin. Both branches require the same timeout result and actual descendant termination. Unexpected causes remain failures. This is a local assertion correction under the approved non-ESRCH error preservation, not a new production exception.

A native diagnostic and [Apple's published XNU group-signal implementation](https://github.com/apple-oss-distributions/xnu/blob/f6217f891ac0bb64f3d375211650a4c1ff8ca1ea/bsd/kern/kern_sig.c#L1675-L1723) establish that a zombie-only group can return `EPERM`. They do not establish the cause of each recorded production `EPERM`. Production still preserves those errors. The separate native diagnostic did not change checkout source.

Final native macOS ARM64 race checks passed ten process-suite repetitions, with 1,000 inherited fork iterations, 1,000 redirected fork iterations and 1,000 exact-command iterations. Actual Linux ARM64 checks passed thirty process-suite repetitions, with 3,000 iterations of each schedule. An additional 100-repeat selection on each platform passed all nine lifetime cases, the flush case, both real timeout cases, normal drain, pipe cleanup/error cases, the original nested-descendant control and both test-group cleanup cases. The existing [FND-21 callback regression](../../../../../../plugins/extension/tools/internal/usecase/tools/bash/timeout_integration_test.go) also passed 100 repetitions on each platform. Linux executed test binaries built from this checkout in the existing Debian image without image pulls, dependency changes or a copied checkout.

The first Linux attempt failed when procfs returned `ESRCH` for a process disappearing during observation. The [test observer](../../../../../../plugins/extension/tools/internal/infra/process/bash/process_observation_integration_test.go) now handles both `ENOENT` and `ESRCH` for that snapshot race. This fixture failure is retained separately from termination RED. Native observations use `ps`; Linux observations use procfs. Neither treats a zombie as live work.

Independent Linux verification then failed two immediate redirected-state assertions. Those logs contain no follow-up observation before cleanup signaled the groups, so they cannot establish why those particular members remained visible. A separate diagnostic retained the immediate assertion and observed an owned member in state `R` with `ShdPnd` equal to `0x100`, then in state `Z` by 2.025333 ms from observation start. No further test signal preceded that transition. The final fork fixture therefore uses a bounded process-state observation barrier, not an unconditional delay or another signal. Its separate Run-completion assertion remains. A persistent live member still fails.

The test cleanup regressions also produced native assertion RED for zombie-only `EPERM` and for a native snapshot using the canceled test context during cleanup. The test helper now accepts `EPERM` only after a bounded observation finds no live group members. Native snapshots use an independent 5-second timeout during cleanup. The zombie and live-member controls passed afterward. These are test-only corrections; production signal order and error policy are unchanged. Temporary diagnostic code was removed from the checkout.

Main inspected the two production files, every added or changed test, resource acquisition/closure, cancellation and output joins, references and the restored producer-error path. Main repeated the native ten-suite and Linux thirty-suite checks above, with 370 and 1,110 exact PASS parent/leaf records respectively. The additional 100-repeat lifetime selection produced 2,000 matching RUN/PASS records on each platform, including the original nested-descendant and test-cleanup controls. Linux binaries were rebuilt from the final source and executed in the recorded existing image.

Main also repeated `task fmt`, `task fix_dry_run` with no proposals, `task lint`, `task test`, `task itest`, `task test-coverage` and `task build`; all passed. Combined coverage was 84.4% against 80.0%. Both broad `-race -count=1` suites passed; integration used `-tags=integration -p 1 -parallel 1`. Two generation runs preserved all 907 hashes. Supplemental unit-tag lint retained all 165 normalized baseline findings and is not reported as passing. The 75-package root graph remains acyclic; public contracts, dependencies and TUI production are unchanged. The separately approved U8 smoke-fixture correction and independent whole-product acceptance remain open.

#### Host-command dispatch confirmation correction

O78-1 authorizes the separate [FND-23 correction](audit.md#fnd-23-non-submit-confirmation-can-precede-input-release) after process commit `40ba3a651d866a323ca166f340ee65a89939b21f`. The original U5 presentation owner remains responsible. The process correction and producer-preservation commit remain unchanged. The correction is implemented and main-agent source and verification checks below are complete. Its separate corrective commit precedes U6/U7 re-entry and U8 fixture work.

`Service.Notify` can receive a correlated Host event before the local `preparedDispatch` result reaches `Service.Complete`. The existing once-only guard covers submitted text, but not other command acknowledgement effects. A confirmed session replacement can therefore render while `interaction.emitting` still discards editor input. Terminal bytes do not expose the later local reset. U8 cannot establish input readiness with a test-only output barrier until this production gap is corrected.

The approved behavior is:

1. Successful local dispatch or the first valid correlated Host notification establishes delivery of a Host command. Apply the command's local acknowledgement effects exactly once, before dependent Host output. Delivery confirmation is not operation success; preserve the separate progress, completion and failure handling.
2. Track that application per pending command for every Host command. Retain D2's single submitted line and request-before-output order. Non-submit confirmation releases input before publishing its confirmed state. Local Quit remains governed by its local result.
3. A late local result completes only its command's correlation bookkeeping and retains its complete diagnostic. It must not clear a newer draft, reset a newer command's dispatch block, repeat submission/replacement or change the shutdown decision. A local send failure without any Host notification keeps the established failure and draft behavior.
4. Keep all state transitions in the serialized presentation owner. Preserve immutable I/O work, selectors, rejected resume, two-stage navigation, foreground cancellation and cause-based persistence cleanup. Add no queue, retry, input replay, sleep, protocol field, test API, dependency or new owner.

Use the original `SubAgentCoderComplex` presentation instance for this same command-lifecycle correction in U5, not the completed process instance. Require executable RED/GREEN for non-submit confirmation followed by actual input, late success/failure, a newer draft and a newer in-flight command. Exercise only events allowed by each command contract. Preserve the existing D2, local Quit, dispatch-first and semantic-cleanup controls. Main source/reference review, required tasks, uncached races/repetitions and generation checks precede one complete corrective commit.

The [presentation lifecycle](../../../../../../plugins/ui/tui/internal/usecase/presentation/commands.go) now records `pendingCommand.acknowledgementApplied` for all Host commands. `Service.applyAcknowledgement` applies local delivery effects before correlated output; `Service.Complete` applies them on dispatch-first success. Later results retain diagnostics without repeating those effects. A late local error after Host progress cannot undo delivery proof, so terminal correlation and the foreground target remain until the Host outcome. Local Quit still uses its own result.

The [command confirmation tests](../../../../../../plugins/ui/tui/internal/usecase/presentation/command_confirmation_test.go) and payload-free lifecycle cases produced executable assertion RED for 16 of 18 leaf cases against the original `commands.go`; both dispatch-first preservation controls passed. All 18 passed after correction. The cases cover replacement, information, Host rejection, selector-driven resume, navigation progress and terminal metadata, late results and actual editing during a newer dispatch. The [existing command lifecycle tests](../../../../../../plugins/ui/tui/internal/usecase/presentation/command_lifecycle_test.go) also cover both arrival orders for payload-free authentication completion, independent connection state, D2, local Quit, failed local dispatch and foreground cancellation.

Local required tasks passed with 84.6% coverage against 80.0%. Local uncached broad unit and integration race runs produced 1,375 and 922 matching RUN/PASS records. The corrected schedules and lifecycle controls passed 100 repetitions with 4,600 matching RUN/PASS records. Two generation runs preserved all 908 repository file hashes. TUI unit-tag lint passed; full supplemental unit-tag lint retained exactly the 165 normalized baseline findings and is not a passing check. The separate smoke fixture, process code and prior producer/runtime corrections are unchanged. Main verification, the separate corrective commit and final acceptance remain pending.

Main inspected the acknowledgement guard, all local effect branches, late-error correlation, input gating and the final contract-valid fixtures. The final RED has 16 failing and two passing leaf cases; the identical GREEN has all 18 passing. Main repeated the 100-run ordering selection with 4,600 matching RUN/PASS records and 2,900 leaf passes. Main also repeated `task fmt`, `task fix_dry_run` with no proposals, `task lint`, `task test`, `task itest`, `task test-coverage` and `task build`; all passed, with coverage at 84.5%. Both broad uncached race suites passed. Two generation runs preserved all 908 hashes, and supplemental unit-tag lint retained exactly the 165 normalized baseline findings. The 75-package root graph is acyclic. No public contract, dependency, process source or smoke fixture changed.

After U6/U7 preservation re-entry, the [U8 fixture correction](u8-evidence.md#smoke-fixture-correction) adds the separately approved barriers. `prepareSubmit.Release` publishes Idle after `RunPrepared` releases admission, so a checkpoint during Running identifies a valid post-run barrier. With FND-23 corrected, confirmed replacement also establishes readiness for the next input. Local and main-agent required checks, repeated actual smoke runs, and current source/assertion accounting are recorded there. Fresh independent whole-product checks/review and explicit final acceptance remain mandatory.
...
The [product PRD](../../prd.md#context-and-sessions) and [architecture data model](../../architecture.md#data-models) define implicit-root session behavior. The architecture [programming interfaces](../../architecture.md#programming-interfaces) and [contracts](../../architecture.md#contracts), the [PHS-06 ticket](../06-context-compaction-retry-control/ticket.md), and the [delivery plan](../../delivery-plan.md) define model-execution ownership and phase allocation. The [PRD error semantics](../../prd.md#error-semantics) and [domain glossary](../../../../../terms.md) define normative error behavior and terms.

**Implicit-root sessions.** [`session.Tree`](../../../../../../host/internal/domain/session/tree.go) represents an existing active entry with `mo.Some(id)` and the implicit root with `mo.None[string]()`. `session.NewTree` accepts stored entries at the implicit root, and `Tree.Clone` retains both the entries and this active position. At the implicit root, the active branch and provider history are empty. Public [`sessions.Service.CloneActive`](../../../../../../host/internal/usecase/host/sessions/replacement_operations.go) copies that empty active branch into an empty replacement session.

Navigation to a root user or model-visible extension entry returns the absent parent as the destination and the selected text as the next input. Persistence replay retains the implicit-root position, and the next append creates and activates a new root entry. [`tree_test.go`](../../../../../../host/internal/domain/session/tree_test.go), [`replacement_operations_test.go`](../../../../../../host/internal/usecase/host/sessions/replacement_operations_test.go), and [`replacement_restart_integration_test.go`](../../../../../../host/internal/infra/persistence/sessions/replacement_restart_integration_test.go) cover construction, cloning, navigation, restart, empty history, labels, and the next append.

**Host model execution.** One [`modelexecution.Service`](../../../../../../host/internal/usecase/host/modelexecution/service.go) implements the Agent Core stream and configured-request contracts for Extension Context and session-tree summarization. The package owns one raw [`ProviderAttempt`](../../../../../../host/internal/usecase/host/modelexecution/contracts.go) per catalogue binding. Codex and OpenAI-compatible drivers implement this raw contract rather than a second configured-request path.

The Agent Core stream resolves a binding without credential preflight. Configured requests use [`providers.Catalog.ResolveConfiguredBinding`](../../../../../../host/internal/usecase/host/providers/request.go) for credential preflight. The service clones history, passes tools only to Agent Core, forwards stream events, and performs configured-response terminal reduction once. Each logical request performs one raw attempt. PHS-06 adds retry policy inside this owner without changing consumers or drivers. [`modelexecution/service_test.go`](../../../../../../host/internal/usecase/host/modelexecution/service_test.go) covers one-attempt streaming, detached configured responses, missing terminals, and provider failure preservation.

**Mixed provider cancellation and error projections.** Provider drivers classify the complete acquired error tree through [`errtree.AllLeavesMatch`](../../../../../../host/internal/errtree/errtree.go). Pure cancellation keeps the canonical canceled result. A mixed tree retains cancellation and each independent provider, callback, finalization, or terminal-assembly cause. Codex constructs a provider source before content finalization, so a later finalization failure cannot replace that source.

Internal returns preserve the original tree for `errors.Is` and `errors.As`. Agent Core terminal text uses canonical cancellation text only for pure cancellation and uses the complete mixed text otherwise. Programmatic Control and Host UI classify pure cancellation but return the original mixed error object and its stable failure category. [`codex/failure_test.go`](../../../../../../host/internal/infra/providers/openai/codex/failure_test.go), [`compatible/stream_failure_test.go`](../../../../../../host/internal/infra/providers/openai/compatible/stream_failure_test.go), [`agent/run/cancellation_test.go`](../../../../../../host/internal/usecase/agent/run/cancellation_test.go), [`programmatic/prepared_test.go`](../../../../../../host/internal/usecase/host/programmatic/prepared_test.go), and [`ui/prepared_operations_test.go`](../../../../../../host/internal/usecase/host/ui/prepared_operations_test.go) cover acquired provider sources, public text, exact mixed-error identity, and category projection.

**Process-group portability.** [`processgroup.New` for Linux](../../../../../../internal/processgroup/sysprocattr_linux.go) and [`processgroup.New` for Darwin](../../../../../../internal/processgroup/sysprocattr_darwin.go) own exhaustive platform-specific `syscall.SysProcAttr` construction. The [bash process adapter](../../../../../../plugins/extension/tools/internal/infra/process/bash/service.go), its [cleanup integration test](../../../../../../plugins/extension/tools/internal/infra/process/bash/cleanup_integration_test.go), and [TUI process test support](../../../../../../internal/testsupport/tui/tui.go) use this constructor instead of partial platform-dependent literals.

**Extension registration publication.** The Extension SDK server commits registration handlers and readiness before the successful registration response enters raw transport `Send`. Therefore, client observation of registration success implies that the server accepts valid registered operations. [`TestServerPublishesRegistrationBeforeSuccessfulSendReturns`](../../../../../../sdk/plugins/extension/v1/server_test.go) covers immediate Execute after the response becomes observable while raw `Send` remains blocked. A registration-response send failure follows the connection-cleanup decisions below.

**Connection cleanup.** Programmatic Control and the Extension SDK server wrap raw server sends with `operation.SendWithContext`. On fatal failure, each handler stops admission, cancels work, joins context-aware application work and `operation.Writer.Run`, collects acquired sources, and returns. Neither server uses a production cleanup timeout or waits for nested raw transport goroutines; handler return lets gRPC cancel those raw calls. The UI SDK server keeps its context-aware send and logical-cleanup path, joins logical writer and operation bookkeeping before handler return, and adds no production cleanup timeout.

Host UI fatal cleanup invokes the runtime-owned [`Service.Cancel`](../../../../../../host/internal/infra/plugins/ui/runtime/operations.go) operation before transport-dependent waits. [`Service.closeFailedOperations`](../../../../../../host/internal/controller/ui/operations.go) joins the writer before `CloseSend`, then joins receive work. Host UI unsuccessful initialization uses one shared one-second grace. Caller cancellation and initialization writer failure start the grace before cleanup joins original transport calls; another unsuccessful result starts it before the close handshake. Grace expiry cancels the actual RPC and releases blocked raw transport calls. Cleanup joins the original send and receive calls, retains their acquired errors, and uses the remaining grace for `CloseConnection`.

Extension SDK client normal close gives the close protocol one second. When that grace expires, [`Connection.Close`](../../../../../../sdk/plugins/extension/v1/host.go) cancels the actual RPC and retains the timeout in completion reporting. Its fatal path keeps immediate RPC cancellation before joins.

Regression evidence covers these boundaries:

- [`TestFatalCleanupReturnsBeforeUnreadRawTransport`](../../../../../../host/internal/controller/programmatic/real_grpc_cleanup_integration_test.go), [`TestBlockedTransportFailureReturnsExtensionHandler`](../../../../../../sdk/plugins/extension/v1/server_blocked_transport_integration_test.go), and [`TestBlockedTransportFailureReturnsUIServerHandler`](../../../../../../sdk/plugins/ui/v1/server_blocked_transport_integration_test.go) cover server handler return after logical cleanup.
- [`TestFatalBlockedHostUISendCancelsRPCBeforeCloseSend`](../../../../../../host/internal/infra/plugins/ui/runtime/client_cleanup_integration_test.go) and [`TestConnectionCloseCancelsBlockedRawSendAfterGrace`](../../../../../../sdk/plugins/extension/v1/client_close_blocked_transport_integration_test.go) cover actual RPC cancellation for fatal Host UI cleanup and Extension SDK client close.
- [`TestCanceledBlockedInitializationUsesOneGrace`](../../../../../../host/internal/infra/plugins/ui/runtime/initialization_cleanup_integration_test.go) and [`TestCanceledInitializationCompletesResponsiveHandshake`](../../../../../../host/internal/infra/plugins/ui/runtime/initialization_cleanup_integration_test.go) cover blocked and responsive Host UI initialization cleanup with one grace.
- [`TestCallerCancellationStartsInitializationGraceBeforeWriterJoin`](../../../../../../host/internal/infra/plugins/ui/runtime/initialization_test.go) and [`TestWriterFailureStartsInitializationGraceBeforeReceiveJoin`](../../../../../../host/internal/infra/plugins/ui/runtime/initialization_test.go) cover grace start before blocked transport joins. [`TestUnsuccessfulInitializationCleanupUsesGraceThenCancelsRPC`](../../../../../../host/internal/infra/plugins/ui/runtime/client_cleanup_integration_test.go) covers expiry, actual RPC cancellation, and the public timeout result.

The [production implementation commits](#status-and-scope) provide the correction inventory. The [roadmap](../../../../../roadmap.md) and [delivery plan](../../delivery-plan.md) retain the phase status without defining these decisions again.
...
None. Required verification, independent whole-product review, and explicit final acceptance remain completion gates rather than open design questions.
