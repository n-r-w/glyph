...
- NFQ-04: Extension-originated errors shall follow the shared [Error Semantics](../../prd.md#error-semantics). Transport message-size and queue limits shall not redefine complete error text.
...
- ACC-02: Active-session or extension-runtime replacement makes every operation through the preceding extension context fail with `STALE_CONTEXT` and complete error text. No stale operation commits session or selection state. After switching from session A to B and back to A, the old A context remains stale and the new A context works; their `context_id` values differ.
...
- ACC-05: Glyph client delivery precedes extension observer delivery for each Agent Core lifecycle event. Observers run in registration order before Agent Core processes the next event. An observer error reaches the Glyph client with its complete text, stops later observers and the operation invoking them, and causes no retry, as required by [PRD FRQ-04](prd.md#requirements). Committed state is retained.
- ACC-06: Two successful selection handlers receive the same original target selection, while the second receives the current target selection returned by the first. Preserve, replace, reject, invalid-action, ordinary-error, cancellation, and unavailable-runtime paths produce the outcomes defined by [PRD FRQ-05 through FRQ-07](prd.md#requirements). A handler error or invalid action invokes no later handler and causes no retry.
- ACC-07: Host validates provider-model existence, reasoning support, and credentials before one atomic selection commit. Rejection, cancellation, stale context, final validation failure, handler error, invalid handler action, or a runtime crash before commit preserves active model selection and emits no selection event. A successful commit emits events only for changed values, with reasoning selection before model selection when both change. Post-commit observer or delivery errors do not roll back the selection; an observer error stops later observers and exposes complete error text with the committed outcome.
- ACC-08: Both extension entry types attach at the active position, including the implicit root, survive restart, and preserve exact extension ID, entry type, parent, timestamp, and payload. Only model-visible extension messages enter model context.
...
- ACC-11: Selecting a model-visible extension message with either client visibility uses its parent as the navigation destination and returns exact text as next input without starting Agent Core. An absent parent selects the implicit root. Without a branch summary, the destination becomes the active position, with no active leaf at the implicit root. With a branch summary, the new `BranchSummaryEntry` becomes the active leaf.
- ACC-11.1: Navigation publishes its committed tree and active branch as operation progress before post-commit observers run. When an observer appends a message and waits for completion, both Glyph client contracts deliver the navigation snapshot before `SessionEntryAdded`. Terminal navigation completion preserves the appended message in client state and returns next input, navigation commit metadata, and issues without another replacement snapshot.
