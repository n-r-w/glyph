...
- ACC-07: Host validates provider-model existence, reasoning support, and credentials before one atomic selection commit. Rejection, cancellation, stale context, final validation failure, or a runtime crash during a handler invocation before commit preserves active model selection and emits no selection event. Ordinary handler errors follow PRD FRQ-07. A successful commit emits events only for changed values, with reasoning selection before model selection when both change. Post-commit observer or delivery errors do not roll back the selection.
