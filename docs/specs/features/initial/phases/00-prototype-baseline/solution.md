...
- DEC-55: A provider-neutral tool call contains a call ID, tool name, and JSON-compatible argument map supporting strings, numbers, booleans, null, arrays, and nested maps. Codex decodes function arguments with `encoding/json/v2` only at terminal completion. Host validates final arguments once against its cached schema before opening the extension `Execute` RPC.
...
- REF-01: `docs/specs/features/initial/phases/00-prototype-baseline/baseline-prd.md` — approved prototype requirements.
