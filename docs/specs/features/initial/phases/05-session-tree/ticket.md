...
The completed [PHS-05.2](../05.2-branch-summary-extension-control/ticket.md) supersedes this phase's unconditional summary-model validation and model-only result attribution. It also owns complete error text for session-tree handlers before PHS-07 starts.
...
- FRQ-02.6: Request and result handlers shall run in registration order with immutable original and current values. A handler error or invalid action shall stop navigation under the [extension failure rule](../../prd.md#extension-failure-rule), report the complete error text, invoke no later handler, and perform no retry. Cancellation shall stop the operation.
...
- ACC-06: A request or result handler error, invalid handler action, cancellation, inconsistent no-summary result, model failure, invalid final result, or persistence failure preserves the active leaf and writes no `BranchSummaryEntry`. A handler failure invokes no later handler and performs no retry. A successful commit emits one `session_tree` event.
