...
- SOL-04: For a replaceable extension operation, the standard implementation runs only when its handlers finish successfully without an extension-provided result. Observers, transformers, and gates have no result-based fallback. Host validates every final result before state commit, persistence, or infrastructure dispatch.
...
- CMP-15.1: Bundled compaction extension. This extension owns the standard context-summary algorithm, prompts, and model requests through the ordinary Extension Contract. Host applies and persists its result under the [compaction requirements](phases/06-context-compaction-retry-control/ticket.md#functional-requirements); Host contains no context-summary generation fallback.
...
- STP-16: After request handlers finish successfully without a result, Host invokes the single registered compaction generator through the public contract. The bundled extension supplies the default generator; a custom extension can replace it. A supplied result bypasses generator lookup and invocation. Zero or multiple generators fail rather than selecting by load order. Result handlers transform the produced result in order. Failure stops compaction without a Host fallback.
- STP-17: Host validates and atomically persists the final compaction entry. Host supplies the compacted model-visible projection to subsequent model requests while original session entries remain stored.
