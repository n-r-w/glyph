...
- CMP-07: Session subsystem. `host/internal/domain/session` owns session and entry models. `host/internal/usecase/host/sessions` and `host/internal/usecase/host/sessioncontrol` own implemented active-session operations. `host/internal/usecase/host/sessiontree` owns implemented tree navigation, branch summarization, and session-tree handler orchestration. The logical Host compaction component owns context compaction.
