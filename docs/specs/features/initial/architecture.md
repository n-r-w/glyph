...
- DEF-08: Observer extension point. A notification whose handlers cannot replace observed data or return a rejection action. Observer failures stop the invoking operation under the [extension failure rule](prd.md#extension-failure-rule) without undoing committed state.
...
- DEF-11: Replaceable extension operation. An operation whose handlers can provide a complete result and skip its standard implementation.
...
- CTX-03: Agent Core requests effective context containing prepared provider-neutral history, one logical model execution, tool execution, history changes, and event delivery through consumer-owned Go interfaces. Host adapters satisfy those interfaces without exposing compaction policy.
...
- SOL-01: Host controllers validate external commands and invoke Host use cases. Host-owned capability use cases coordinate their extension handlers and validate their final state. Host runtime and session owners validate transport shape and structural/session invariants for extension-owned compaction without selecting its policy.
- SOL-02: Agent Core runs one provider-neutral loop. It asks Host adapters for effective context containing prepared history, model execution, and tool execution, then applies terminal results to the active run. It has no compaction policy or compaction-specific orchestration responsibility.
- SOL-03: Every transforming extension point starts with equal original and current values. Each handler receives the immutable original value and the current value from preceding successful handlers. Handler failures follow the PRD's [extension failure rule](prd.md#extension-failure-rule).
- SOL-04: For a replaceable extension operation, the capability owner invokes the standard implementation only when its handlers finish successfully without an extension-provided result. Observers, transformers, and gates have no result-based fallback. Host validates transport shape and the owning state boundary before commit, persistence, or infrastructure dispatch without replacing extension-owned policy.
- SOL-05: Each Host-owned capability use case owns its extension-handler ordering and final validation. The compaction extension owns compaction policy and ordered request/result composition; Host runtime and session owners provide invocation, structural validity, durable mutation, projection, publication, and accounting services without duplicating that policy. Agent Core receives only the minimum provider-neutral contracts required by its loop.
...
- CMP-04: Agent Core. `host/internal/usecase/agent/run` owns the run state machine, model and tool loop, ordered agent events, cancellation, and terminal run outcome. Core directly implements run control's execution and settlement contract. Its execution result reports whether the run entered awaiting-settlement state, including cancellation before the first history append. A failed begin reports no settlement transition. Core's outgoing interfaces describe effective context containing prepared history, logical model execution, tools, history mutation, and event delivery. None exposes compaction policy or compaction-specific orchestration.
...
- CMP-06: Model subsystem. `host/internal/usecase/host/providers` owns the implemented provider catalogue, provider-neutral model descriptors, active model selection, reasoning choice, credential preflight, and atomic selection commit. The logical Host provider-authentication component owns the client-neutral operation that starts or retries authentication for one configured provider. `host/internal/usecase/host/modelexecution` owns logical model execution, one raw provider-attempt contract, provider dispatch, provider middleware, retry decisions, and terminal model-call reduction.
- CMP-06.1: Extension model access. PHS-06 separates extension-facing model/provider catalogue queries and configured-model request operations into `host/internal/usecase/host/extensionmodels`. This owner consumes context validation and model execution through its own interfaces. `extensioncontext` retains context issuance, binding validation, and session operations; `modelexecution` retains shared model execution and does not import `extensioncontext`.
- CMP-07: Session subsystem. `host/internal/domain/session` owns shared session information, entries, trees, and accounting values. `host/internal/usecase/host/sessions` owns active state, incarnation, prepared-history projection, durable mutations, structural and session validity, restoration, navigation projection, and committed-entry publication. Original entries remain stored when a compaction entry replaces an older prefix in prepared history. Host UI and Host Programmatic own their active-session and navigation contracts, stored-list results, and public response projections. Sessions implements their state and history queries directly. `host/internal/usecase/host/sessiontree` implements their navigation contracts and owns handler policy, summarization, final validation, and its outgoing atomic commit contract. No session or Host component owns compaction triggering, sizing, retained-boundary selection, generation, or compaction-specific overflow policy.
...
- ENT-05: A session tree contains parent-linked entries and one active position. The active position is an existing entry or the implicit root before every root entry. `active leaf` means the entry-valued active position. The active branch is empty at the implicit root. Entry variants include user messages, model responses, tool results, extension entries, `BranchSummaryEntry`, and compaction entries.
...
- APC-01: Agent Core declares its effective-context interface in `host/internal/usecase/agent/run`. A Host adapter supplies prepared provider-neutral history after session projection and applicable context transformations without exposing compaction, handler count, order, process identity, protobuf, or gRPC.
...
- STP-03: Agent Core requests effective context containing prepared model history through APC-01 and submits one logical model request through APC-02. Agent Core does not know whether a compaction entry contributes to that history.
...
- STP-08: For each Host-owned transforming extension point, Host creates DEF-04 and invokes handlers in registration order. The compaction flow assigns equivalent composition to the compaction extension.
- STP-09: A handler preserves or replaces the current value from the original value, the current value, or both. An invalid action or handler error stops the operation under the PRD's [extension failure rule](prd.md#extension-failure-rule); later handlers do not run and no retry occurs.
...
- STP-14: Sessiontree validates the final tree target and `BranchSummaryEntry`. Sessions checks the expected active position, persists navigation and summary together, updates active state and history, and enqueues the immutable committed tree under one lock. A root entry's absent parent selects the implicit root, which produces empty active history and an absent active-leaf result field. Wire acknowledgement and `session_tree` observers run after commit protection is released. Enqueue failure returns committed metadata and a delivery issue without rollback. Cancellation or invalid final state changes neither the active position nor persisted entries.
...
- STP-15: The policy-owning compaction extension decides whether to compact, how to size context, which boundary to retain, and how to respond to compaction-specific provider overflow. It establishes immutable original and current requests, then coordinates request handlers in registration order. Each handler receives the immutable original request, the current request, and the current result and can independently preserve or replace the request and preserve, set, replace, or clear the result, or cancel.
- STP-16: A complete current result bypasses generation. Without a result, the policy-owning extension invokes exactly one registered generator; zero or multiple generators fail without a Host fallback or load-order selection. Result handlers then receive the immutable original request, final current request, immutable original result, and composed current result in registration order and can preserve or replace the result or cancel. The exact invocation mechanism remains the technical design work identified by [QST-01](phases/06-context-compaction-retry-control/ticket.md#qst-01-minimal-compaction-integration-contract).
- STP-17: Host provides extension invocation, cancellation, session-bound history access, usage observations, model access, durable mutation, publication, and existing accounting without running duplicate compaction policy. The session owner checks structural and session validity and atomically appends the extension-selected compaction entry. Prepared-history projection uses the latest applicable summary followed by its preserved suffix while original entries remain stored. Restore and navigation rebuild the same projection.
...
- STP-22: Ordered selection handlers preserve, replace, or reject the current selection. Rejection stops later selection handlers. A handler error or invalid action stops selection under the PRD's [extension failure rule](prd.md#extension-failure-rule). Host validates model existence, reasoning capability, and authentication only after the handler chain completes without rejection or failure.
...
- DEC-02: Put extension runtime coordination in Host while leaving extension-owned policy and compaction request/result composition in the compaction extension. This keeps plugin transport and failure mapping out of Agent Core without moving compaction policy into Host.
...
- DEC-04: Invoke the standard implementation of a replaceable extension operation only when its handlers finish successfully without a result. This gives extensions replacement control without applying result fallback to observers, transformers, or gates.
...
- RSK-01: Host orchestration packages can grow into one dependency hub. Keep extension runtime management separate from model execution, sessions, tools, selection, commands, resources, and client delivery. Host-owned capabilities retain their policy; the compaction extension retains compaction policy and uses Host runtime and session services.
...
- RSK-03: Extension-produced data can violate session or provider invariants. Host validates transport shape and the owning session or provider invariants immediately before the atomic commit or infrastructure call without adding capability policy.
...
### QST-01: Compaction integration contract

The minimal mechanism and call flow through which the policy-owning compaction extension invokes participating request, generator, and result capabilities while owning trigger, sizing, boundary, generation, and overflow policy and using Host runtime and session services remains TBD. PHS-06 shall resolve the owning [ticket question](phases/06-context-compaction-retry-control/ticket.md#qst-01-minimal-compaction-integration-contract) before implementation migration.
