...
func Reject(code string, cause error) error {
	if code == "" {
		panic("extension rejection code is required")
	}
	if cause == nil {
		panic("extension rejection cause is required")
	}
...
func Fail(code string, cause error) error {
	if code == "" {
		panic("extension failure code is required")
	}
	if cause == nil {
		panic("extension failure cause is required")
	}
...
// newRemoteRejection reconstructs a rejection received from the extension.
func newRemoteRejection(code, message string) error {
	return &RejectionError{code: code, cause: errors.New(message)}
}

// newRemoteFailure reconstructs a failure received from the extension.
...
func newCanceledError() error {
	return &CanceledError{cause: fmt.Errorf("extension operation canceled: %w", context.Canceled)}
}
