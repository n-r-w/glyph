...
func TestServiceAssignsToolCompletionStatusAndResultContentOnce(t *testing.T) {
	...
	service := New()
	state := service.Apply(presentationdomain.State{}, testToolEndedEvent("read", "completed", false))
...
func TestServiceRendersOneSafeErrorAcrossTerminalLifecycleEvents(t *testing.T) {
	...
	for _, event := range []presentationdomain.Event{
		...
		testFailureEvent(presentationdomain.EventTurnEnded, "Provider failed."),
		{
			RestoredTranscript:   nil,
...
func TestServiceRetainsTranscriptAcrossSettlementAndSecondTurn(t *testing.T) {
	...
	service := New()
	state := service.Apply(presentationdomain.State{}, testAvailabilityEvent(
		presentationdomain.EventInitialization, presentationdomain.AvailabilityIdle,
	))
	// Act by settling the first turn and applying the second turn.
	state = service.Apply(state, testAvailabilityEvent(
		presentationdomain.EventAvailability, presentationdomain.AvailabilityRunning,
	))
