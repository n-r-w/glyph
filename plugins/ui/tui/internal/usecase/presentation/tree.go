...
// VisibleEntry contains semantic tree visibility and selection data, without terminal geometry.
type VisibleEntry struct {
	// Entry is the visible committed entry.
	Entry TreeEntry
	// ParentID identifies the nearest visible parent, if any.
	ParentID mo.Option[string]
	// ActivePath identifies membership in the active branch.
	ActivePath bool
	// ActiveLeaf identifies the active entry.
	ActiveLeaf bool
	// Context identifies a nonmatching search ancestor.
	Context bool
	// Folded identifies an entry whose descendants are hidden.
	Folded bool
	// HasChildren reports complete-tree children even when filtered out.
	HasChildren bool
}
...
func (panel treePanel) visibleEntries() []VisibleEntry {
	...
	rows := make([]VisibleEntry, 0, len(visibleEntries))
	for entryIndex := range visibleEntries {
		entry := &visibleEntries[entryIndex]
		parentID, parentPresent := nearestVisibleParent(*entry, entriesByID, visibleIDs)
		_, direct := directMatches[entry.ID]
		_, onActivePath := activePath[entry.ID]
		_, folded := panel.Folded[entry.ID]
		rows = append(rows, VisibleEntry{
			Entry:       *entry,
			ParentID:    mo.TupleToOption(parentID, parentPresent),
...
func (panel *treePanel) MoveSelection(delta int) {
	...
	if selectedID, present := panel.SelectedID.Get(); present {
		if selectedIndex := slices.IndexFunc(
			rows,
			func(row VisibleEntry) bool { return row.Entry.ID == selectedID },
		); selectedIndex >= 0 {
			index = selectedIndex
		}
	}
