...
func TestServiceGrepReturnsRelativeLinesAndContext(t *testing.T) {
	...
	result, err := New().Grep(t.Context(), searchtool.GrepCommand{
		Pattern: "match", Path: ".", Glob: "", IgnoreCase: false, Literal: false, Context: 1,
		Limit: mo.EmptyableToOption[uint](100),
	})
...
func TestServiceGrepSkipsSymlinksAndReportsObservedMatchLimit(t *testing.T) {
	...
	result, err := New().Grep(t.Context(), searchtool.GrepCommand{
		...
		Limit: mo.EmptyableToOption[uint](1),
	})
...
func TestServiceFindReturnsLinkWithoutEnteringAndUsesRecursiveGlob(t *testing.T) {
	...
	result, err := New().Find(t.Context(), searchtool.FindCommand{
		Pattern: "**/*.go", Path: ".", Limit: mo.EmptyableToOption[uint](1000),
	})
...
func TestServiceEscapesLineBreaksInDisplayedNames(t *testing.T) {
	...
	findResult, findErr := service.Find(t.Context(), searchtool.FindCommand{
		Pattern: "*.txt", Path: ".", Limit: mo.EmptyableToOption[uint](1000),
	})
	listResult, listErr := service.List(t.Context(), searchtool.ListCommand{
		Path: ".", Limit: mo.EmptyableToOption[uint](500),
	})
